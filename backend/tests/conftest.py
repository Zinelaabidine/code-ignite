...
The three required `CODEIGNITE_*` env vars (`aws_region`, `jobs_bucket`,
`runs_queue_url` — see `config.py`) are set via `pytest-env` in
`pyproject.toml`, not here, since `config.py` instantiates `Settings()`
eagerly at import time and needs a value before the first test module even
finishes importing `codeignite.config`.
"""
