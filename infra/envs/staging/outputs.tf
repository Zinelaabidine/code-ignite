output "site_url" {
  description = "Public HTTPS URL of the staging site."
  value       = module.static_site.site_url
}

output "s3_bucket_name" {
  description = "S3 bucket holding the staging static site build."
  value       = module.static_site.s3_bucket_name
}

output "cloudfront_distribution_id" {
  description = "CloudFront distribution ID for staging (used for cache invalidation on deploy)."
  value       = module.static_site.cloudfront_distribution_id
}

output "github_oidc_deploy_role_arn" {
  description = "IAM role GitHub Actions assumes to deploy staging."
