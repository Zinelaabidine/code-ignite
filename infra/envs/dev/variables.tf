...
variable "domain_name_override" {
  description = "Optional FQDN override. When null, defaults to \"{project_name}-dev.{hosted_zone_name}\" in dev, \"{project_name}-staging.{hosted_zone_name}\" in staging, and \"{project_name}.{hosted_zone_name}\" in prod."
  type        = string
  default     = null
}
